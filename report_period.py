...
if not os.path.exists("LOGS_Period"):
    os.makedirs("LOGS_Period")
output = open("LOGS_Period/logs_"+time_start+"_"+time_stop+".txt", "w")

tmp_file = open("tmp.txt", "w")

year_start=time_start[0]+time_start[1]+time_start[2]+time_start[3]
day_start=time_start[4]+time_start[5]
month_start=time_start[6]+time_start[7]

year_stop=time_stop[0]+time_stop[1]+time_stop[2]+time_stop[3]
day_stop=time_stop[4]+time_stop[5]
month_stop=time_stop[6]+time_stop[7]
...
for user in unique_users:
    ...
    for file in files_period:
        ...
        job_file = []
        with open("logs/"+file, "r") as f:
            tmp_file.write("reading file " + file +" for user "+name+" "+surname+ "\n")
            tmp = f.read().split('\n')
        ...
        if unique_jobs_file.__len__() != 0:
                                ...
                                if tk[5].split("=")[0] == "Exit_status":
                                    ...
                                    for jobQ, queueQ in jobs_and_queue:
                                        if jobQ == job:
                                            right_queue = queueQ
                                    output.write(
                                        "JOB: " + job + ", date: " + day + "/" + month + "/" + year + ", queue: " + right_queue + ", " + str(
                                            cputime) + ", " + str(
            ...
            if flag == 0:
                ...
                output.write("\n\nTotal cputime of file " + day + "/" + month + "/" + year + ": " + str(
                    total_cputime_file) + ". Total walltime of file is: " + str(total_walltime_file) + "\n")
    ...
    output.write("\nTOTAL CPU TIME = " + str(final_cputime) + ", TOTAL WALLTIME = " + str(
        final_walltime) + ",  TOTAL NUMBER OF JOBS = " + str(unique_jobs.__len__()) + "\n")
